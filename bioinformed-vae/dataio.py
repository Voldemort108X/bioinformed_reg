...
def load_data(data_path, filename, size, rand_frame=None):
    ...
    image_sa = image_sa[np.newaxis] # sa_image (30 time axis)

    nim = nib.load(join(data_path, filename, 'sa_'+'ED'+'.nii.gz'))
    image = nim.get_data()[:, :, :]
    image = np.array(image, dtype='float32') # seems to be the target image

    nim_seg = nib.load(join(data_path, filename, 'label_sa_'+'ED'+'.nii.gz'))
    seg = nim_seg.get_data()[:, :, :] # segmentation mask
    ...
    mask = np.array(mask[np.newaxis], dtype='int16') # create the myocardial mask
